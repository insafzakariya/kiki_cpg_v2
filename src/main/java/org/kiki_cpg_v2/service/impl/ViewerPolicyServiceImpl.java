...
public class ViewerPolicyServiceImpl implements ViewerPolicyService {
	...
	public String updateViewerPolicy(ViewerPolicyUpdateRequestDto viewerPolicyUpdateRequestDto) {
		...
		if (viewerPackageEntity != null && viewerPackageEntity.getPackageEntity() != null) {

			if (viewerPackageEntity.getPackageEntity().getId() == viewerPolicyUpdateRequestDto.getPackageId()) {

				List<PackagePolicyEntity> packagePolicyEntities = new ArrayList<>();
				for (PackagePolicyEntity packagePolicyEntity : viewerPackageEntity.getPackageEntity()
				...
				packagePolicyEntities.forEach(e -> {
					viewerPolicyEntities.add(
							getViewerPolicyEntity(e, viewerPackageEntity, viewerPolicyUpdateRequestDto.getViewerId()));
				});
				...
				if(viewerPackageRepository.saveAll(viewerPackageEntities) != null) {
					PackageEntity packageEntity = packageRepository.findById(viewerPolicyUpdateRequestDto.getPackageId()).get();
					ViewerPackageEntity tempViewerPackageEntity = getViewerPackageEntity(packageEntity , viewerPolicyUpdateRequestDto.getViewerId());
					...
					packageEntity.getPackagePoliciesEntities().forEach(e ->{
						viewerPolicyEntities.add(getViewerPolicyEntity(e, newViewerPackageEntity, viewerPolicyUpdateRequestDto.getViewerId()));
					});
					...
					if(viewerPolicyEntities != null && !viewerPolicyEntities.isEmpty()) {
						...
						return "No policies for package id : " + packageEntity.getId() ;
					}
	...
	public ViewerPolicyEntity getViewerPolicyEntity(PackagePolicyEntity packagePolicyEntity, ViewerPackageEntity viewerPackageEntity,
			Integer viewerId) {
		...
		c.setTime(new Date());
		c.add(Calendar.DATE, viewerPackageEntity.getPackageEntity().getAvailableDays());
